...
void nau8822_spi_transfer(uint8_t data)
{
	; // function to actually bit shift the data uint8_t out

	for (int i = 1; i <= 8; i++)
	{ // setup a loop of 8 iterations, one for each bit
		if (data > 127)
		{											   // test the most significant bit
			gpio_set_level(NAU8822_SPI_DAT_PIN, HIGH); // if it is a 1 (ie. B1XXXXXXX), set the master out pin high
		}
		else
		{
			gpio_set_level(NAU8822_SPI_DAT_PIN, LOW); // if it is not 1 (ie. B0XXXXXXX), set the master out pin low
		}
		gpio_set_level(NAU8822_SPI_CLK_PIN, HIGH); // set clock high, the pot IC will read the bit into its register
		data = data << 1;
		gpio_set_level(NAU8822_SPI_CLK_PIN, LOW); // set clock low, the pot IC will stop reading and prepare for the next iteration (next significant bit
	}
}
...
void nau8822_register_write_spi(uint8_t addr, uint16_t data)
{
	...
	gpio_set_level(NAU8822_CS_PIN, LOW); // pull SS slow to prep other end for transfer

	nau8822_spi_transfer(a);
	nau8822_spi_transfer(b);

	gpio_set_level(NAU8822_CS_PIN, HIGH); // pull ss high to signify end of data transfer;
}
...
void nau8822_register_write(uint8_t addr, uint16_t data)
{
	...
	i2c_master_write_uint8_t(cmd, (ESP_SLAVE_ADDR) | WRITE_BIT, ACK_CHECK_EN);
	i2c_master_write_uint8_t(cmd, ((addr << 1) | (data >> 8)) | WRITE_BIT, ACK_CHECK_EN);
	i2c_master_write_uint8_t(cmd, ((int8_t)(data & 0x00ff)) | WRITE_BIT, ACK_CHECK_EN);
	i2c_master_stop(cmd);
...
void nau8822_dac_gain(uint8_t volume)
{
	...
	nau8822_register_write_spi(45, volume); // LADC a LIN 0 dB
	nau8822_register_write_spi(46, 0x100+volume); // RADC a RIN 0 dB
}

uint8_t nau8822_init(int modo)
{

	// Máscara con los 3 pines
	uint64_t pin_mask = (1ULL << NAU8822_SPI_DAT_PIN) |
						(1ULL << NAU8822_SPI_CLK_PIN) |
						(1ULL << NAU8822_CS_PIN) |
						(1ULL << msi001_cs_pin);
...
frequency is 32x.*/
	...
	case 7: // LIN RIN
		...
		nau8822_register_write_spi(44, 0x044); // CVonecta LIN y RIN sólo al PGA
	...
	case 9:
		...
		nau8822_register_write_spi(52, 0x000);
		nau8822_register_write_spi(53, 0x100);
		nau8822_register_write_spi(54, 0x000);
		nau8822_register_write_spi(55, 0x100); // Vol

		nau8822_register_write_spi(43, 0x020); // Speaker out 0x010, Mute 0x020

		nau8822_register_write_spi(1, 0xFD); // 0x01D
		nau8822_register_write_spi(2, 0x1BF);
		nau8822_register_write_spi(3, 0x1EF);
		nau8822_register_write_spi(56, 0xb00000111); // AUX
		...
		nau8822_register_write_spi(11, 0x0FF);
		nau8822_register_write_spi(12, 0x1FF);

		nau8822_register_write_spi(43, 0x010); // Speaker out 0x010, Mute 0x020

		nau8822_register_write_spi(49, 0x006);

		nau8822_register_write_spi(50, 0x000); // 0x001 /* Left DAC connected to LMIX 0x01f mik na dinamik */
		nau8822_register_write_spi(51, 0x01d); // 0x001 /* Right DAC connected to RMIX 0x01f mik na dinamik !!! */

		nau8822_register_write_spi(52, 0x03F); //  ON / LHP
		nau8822_register_write_spi(53, 0x13F); //  ON / RHP
		nau8822_register_write_spi(3, 0xF);	   // AUX1 AUX2 OF

		nau8822_register_write_spi(44, 0x030); /* LMICN/LMICP is connected to PGA */

		nau8822_register_write_spi(45, 0x03f); // PGA
		nau8822_register_write_spi(46, 0x03f); // PGA

		nau8822_register_write_spi(65, 0x000);
